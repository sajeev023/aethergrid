          ...
          {FACILITIES.map((item, idx) => (
            <Reveal key={item.title} delay={idx * 0.06}>
              <div className="group relative aspect-[4/3] rounded-lg overflow-hidden border border-stone-texture shadow-sm hover:shadow-panel-hover transition-all duration-300">
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-academic-slate/80 via-academic-slate/20 to-transparent" />
                <div className="absolute inset-x-0 bottom-0 p-3">
                  <h3 className="font-serif text-sm font-bold text-white leading-tight mb-1">{item.title}</h3>
                  <p className="text-[10px] leading-4 text-royal-cream/80 font-sans">{item.desc}</p>
                </div>
              </div>
            </Reveal>
          ))}
              ...
              <div className="group bg-white border border-stone-texture rounded-xl overflow-hidden hover:border-heritage-gold/50 hover:shadow-panel-hover transition-all duration-300">
                <div className="relative aspect-[4/3] overflow-hidden">
                  <Image
                    ...
                    className="object-cover transition-transform duration-700 group-hover:scale-[1.04]"
                  />
                  <div className="absolute top-3 right-3 bg-heritage-gold text-white text-[9px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-full">
                    {lab.badge}
                  </div>
                </div>
                <div className="p-4">
                  <h3 className="font-serif text-base font-bold text-academic-slate mb-1 group-hover:text-montfortian-blue transition-colors">
                    {lab.title}
                  </h3>
                  <p className="text-xs leading-5 text-academic-slate/70 font-sans">{lab.desc}</p>
                </div>
              </div>
            </Reveal>
          ))}
            ...
            <Reveal key={item.title} delay={idx * 0.08}>
              <div className="group bg-royal-cream/15 border border-stone-texture rounded-xl overflow-hidden hover:bg-white hover:border-heritage-gold/50 hover:shadow-panel-hover transition-all duration-300 h-full flex flex-col">
                ...
                <div className="p-6 flex flex-col flex-1">
                  ...
                  <h3 className="font-serif text-xl font-bold text-academic-slate mb-2 group-hover:text-montfortian-blue transition-colors">
                    {item.title}
                  </h3>
                  <p className="text-xs leading-5 text-academic-slate/70 font-sans mb-4 flex-1">{item.desc}</p>
                  <div className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-heritage-gold-strong">
