...
const UPLOAD_DIR = path.join(process.cwd(), "public", "images", "faculty");
...
export async function POST(request: NextRequest) {
  ...
  const publicUrl = `/images/faculty/${safeFilename}`;

  const ip = request.headers.get("x-forwarded-for") ?? "unknown";
  logAudit(session.userId, session.username, "faculty_photo_upload", `Uploaded: ${safeFilename}`, ip);
