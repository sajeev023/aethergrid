...
export function Admissions({ activeInst = "lfjc", headingLevel = "h2" }: AdmissionsProps) {
  ...
  return (
    <div id="admissions" className="bg-royal-cream/10">
      ...
      <div className="relative isolate overflow-hidden border-y border-stone-texture py-10 md:py-14">
        <Image
        ...
        <div className="mx-auto max-w-5xl px-5 text-center md:px-8">
          ...
          >
            <motion.p variants={itemVariants} className="text-[11px] font-bold uppercase tracking-[0.24em] text-heritage-gold-strong">
              {instData.admissionsLabel}
            </motion.p>
            <PortalHeading variants={itemVariants} className="mt-3 font-serif text-3xl font-bold leading-[1.1] text-deep-navy md:text-4xl lg:text-5xl tracking-tight">
              Admissions Portal
            </PortalHeading>
            <motion.p variants={itemVariants} className="mx-auto mt-3 max-w-3xl font-editorial text-lg md:text-xl leading-relaxed text-academic-slate/85 italic">
            ...
            <motion.div variants={itemVariants} className="mt-5 flex flex-col justify-center gap-3 sm:flex-row">
              ...
              <Button asChild variant="secondary" size="lg" className="shadow-sm whitespace-normal h-auto py-2.5 text-center text-xs">
      ...
      {/* Eligibility Section */}
      <div id="eligibility" className="mx-auto grid max-w-7xl scroll-mt-28 gap-6 lg:gap-10 px-5 md:px-8 py-10 md:py-14 lg:grid-cols-[0.8fr_1.2fr] items-start border-b border-stone-texture/40">
        <Reveal>
          <div className="lg:sticky lg:top-24">
            ...
            />
            <blockquote className="mt-5 border-l-2 border-heritage-gold pl-4 font-editorial text-lg italic leading-7 text-academic-slate/75">
            ...
            <Card className="bg-white hover:shadow-panel-hover hover:border-heritage-gold/40 transition-all duration-300 border border-stone-texture/60">
              <CardHeader className="p-4 pb-0 md:p-5 md:pb-0">
                <div className="flex items-center gap-3">
                  ...
                  <CardTitle className="text-lg font-serif font-bold text-academic-slate">Academic Requirements</CardTitle>
                </div>
              </CardHeader>
              <CardContent className="p-4 md:p-5 pt-3 md:pt-3">
                <ul className="grid gap-3 text-xs md:text-sm leading-6 text-academic-slate/80 font-sans">
                  {getAcademicRequirements().map((item) => (
                    <li key={item} className="flex gap-3 items-start">
                      <CheckCircle2 className="mt-1 h-4 w-4 shrink-0 text-montfortian-blue" aria-hidden="true" />
                      <span>{item}</span>
                    </li>
                  ))}
                </ul>
            ...
            <Card className="bg-white hover:shadow-panel-hover hover:border-heritage-gold/40 transition-all duration-300 border border-stone-texture/60">
              <CardHeader className="p-4 pb-0 md:p-5 md:pb-0">
                <div className="flex items-center gap-3">
                  ...
                  <CardTitle className="text-lg font-serif font-bold text-academic-slate">The Open-Door Policy</CardTitle>
                </div>
              </CardHeader>
              <CardContent className="p-4 md:p-5 pt-3 md:pt-3">
                <p className="leading-6 text-xs md:text-sm text-academic-slate/85 font-sans">
      ...
      <div id="admission-roadmap" className="section-texture scroll-mt-28 border-b border-stone-texture bg-white py-8 md:py-12">
        <div className="mx-auto max-w-7xl px-5 md:px-8">
          ...
          />
          <div className="mt-6 grid gap-4 grid-cols-2 md:grid-cols-4 relative">
            {instData.admissionsSteps.map((step, index) => (
              <Reveal key={step.title} delay={index * 0.06}>
                <div className="relative h-full border border-stone-texture/60 bg-royal-cream/35 p-5 hover:bg-white hover:border-heritage-gold/50 hover:shadow-panel-hover transition-all duration-300 rounded-xl group flex flex-col justify-between">
                  <div>
                    {/* Watermarked step index */}
                    <span className="absolute right-5 top-5 font-serif text-4xl font-bold italic text-heritage-gold/15 select-none pointer-events-none group-hover:text-heritage-gold/30 group-hover:opacity-100 transition-all duration-500">
                      0{index + 1}
                    </span>
                    
                    <div className="flex h-9 w-9 items-center justify-center rounded-full bg-montfortian-blue text-white text-xs font-bold shadow-md">
                      {index + 1}
                    </div>
                    
                    <h3 className="mt-3 font-serif text-base font-bold text-academic-slate group-hover:text-montfortian-blue transition-colors">
                      {step.title}
                    </h3>

                    <p className="mt-1.5 text-xs leading-5 text-academic-slate/75 font-sans">
                      {step.description}
                    </p>
                  </div>
                </div>
      ...
      <div id="fee-structure" className="scroll-mt-28 border-b border-stone-texture bg-royal-cream/20 py-8 md:py-12">
        <div className="mx-auto max-w-7xl px-5 md:px-8">
          <SectionHeading
          ...
          <div className="mt-6 grid gap-6 md:grid-cols-2">
            ...
            <Reveal className="border border-stone-texture bg-white p-5 rounded-xl shadow-sm">
              ...
              <h3 className="font-serif text-lg font-bold text-academic-slate mb-3">
                Fee Components (2026–27 Session)
              </h3>
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs font-sans border-collapse">
                  <thead>
                    <tr className="border-b border-stone-texture/60 text-heritage-gold-strong font-bold uppercase text-[10px]">
                      <th className="py-2 px-1">Component</th>
                      <th className="py-2 px-1">Applicability</th>
                  ...
                  <tbody className="divide-y divide-stone-texture/30 text-academic-slate">
                    {[
                      ...
                      <tr key={row.component}>
                        <td className="py-2 px-1 font-bold text-montfortian-blue">{row.component}</td>
                        <td className="py-2 px-1">{row.applicability}</td>
                        <td className="py-2 px-1 text-right text-academic-slate/70">{row.basis}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              </div>
              <p className="mt-3 text-[11px] leading-relaxed text-academic-slate/70 font-sans italic">
            ...
            <Reveal delay={0.1} className="border border-stone-texture bg-white p-5 rounded-xl shadow-sm flex flex-col justify-between">
              <div>
                <span className="text-[10px] font-bold uppercase tracking-[0.16em] text-heritage-gold-strong font-sans block mb-1">
                  Merit Concessions
                </span>
                ...
                <ul className="space-y-2.5 text-xs text-academic-slate/80 font-sans">
      ...
      <div id="admissions-calendar" className="scroll-mt-28 border-b border-stone-texture bg-white py-8 md:py-12">
        <div className="mx-auto max-w-7xl px-5 md:px-8">
          <SectionHeading
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-2">
            {/* Calendar */}
            <Reveal className="border border-stone-texture bg-royal-cream/30 p-5 rounded-xl">
              <div className="flex items-center gap-3 mb-4">
                <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-montfortian-blue text-white">
                  <ArrowDown className="h-5 w-5 rotate-[-90deg]" aria-hidden="true" />
                </div>
                <h3 className="font-serif text-lg font-bold text-academic-slate">Admissions Calendar</h3>
              </div>
              <ol className="relative grid gap-4">
                {admissionsCalendar.map((item, index) => (
                  <li key={item.phase} className="flex gap-4">
                    <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-heritage-gold/40 bg-white font-serif text-xs font-bold text-montfortian-blue">
                      {index + 1}
                    </div>
                    <div>
                      <p className="font-serif text-sm font-bold text-academic-slate">{item.phase}</p>
                      <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-heritage-gold-strong font-sans">
                        {item.date}
                      </p>
                      <p className="mt-1 text-xs leading-5 text-academic-slate/70 font-sans">{item.detail}</p>
                    </div>
                  </li>
                ))}
              </ol>
              <p className="mt-4 text-[11px] leading-relaxed text-academic-slate/60 font-sans italic">
            ...
            {/* Documents */}
            <Reveal delay={0.1} className="border border-stone-texture bg-white p-5 rounded-xl">
              <div className="flex items-center gap-3 mb-4">
                <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-royal-cream text-heritage-gold-strong border border-stone-texture/60">
                  <FileText className="h-5 w-5" aria-hidden="true" />
                </div>
                <h3 className="font-serif text-lg font-bold text-academic-slate">Documents Required</h3>
              </div>
              <ul className="grid gap-2.5 text-xs md:text-sm leading-6 text-academic-slate/80 font-sans">
      ...
      {/* Admissions FAQ */}
      <div id="admissions-faq" className="scroll-mt-28 border-b border-stone-texture bg-royal-cream/15 py-8 md:py-12">
        <div className="mx-auto max-w-4xl px-5 md:px-8">
          ...
          <div className="mt-6 grid gap-3">
            {admissionsFaq.map((item) => (
              <details key={item.q} className="group rounded-lg border border-stone-texture/60 bg-white p-4 md:p-5 hover:border-heritage-gold/40 transition-colors">
                <summary className="flex cursor-pointer items-start justify-between gap-4 list-none">
                  <span className="flex items-start gap-3">
                    <HelpCircle className="mt-0.5 h-4 w-4 shrink-0 text-heritage-gold-strong" aria-hidden="true" />
                    <span className="font-serif text-sm md:text-base font-bold text-academic-slate">{item.q}</span>
                  </span>
                  <span className="mt-1 text-heritage-gold-strong transition-transform duration-300 group-open:rotate-45" aria-hidden="true">+</span>
                </summary>
                <p className="mt-3 pl-7 text-xs md:text-sm leading-6 text-academic-slate/75 font-sans">{item.a}</p>
              </details>
            ))}
          </div>
        </div>
      ...
      <div id="admissions-form" className="mx-auto grid max-w-7xl scroll-mt-28 gap-6 lg:gap-10 px-5 md:px-8 py-10 md:py-14 lg:grid-cols-[0.8fr_1.2fr] items-start">
        <Reveal>
          <div className="lg:sticky lg:top-24">
            <SectionHeading
              align="left"
