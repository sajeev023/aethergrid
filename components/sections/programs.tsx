...
export function Programs({ activeInst = "lfjc", headingLevel = "h2" }: ProgramsProps) {
  ...
  return (
    <Section id="programs" variant="default" className="bg-royal-cream border-b border-stone-texture/50 section-texture">
      ...
      {/* Intro banner card */}
      <Reveal className="mx-auto mt-8 grid max-w-5xl gap-4 border border-stone-texture/60 bg-white p-5 shadow-panel md:grid-cols-[0.9fr_1.1fr] rounded-lg">
          <div className="border-l-2 border-heritage-gold pl-4">
            <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-heritage-gold-strong">
              Academic Advising
            </p>
            <p className="mt-1.5 font-serif text-xl font-semibold leading-tight text-academic-slate">
              Choose your stream based on academic interest, aptitude, and future ambitions.
            </p>
          </div>
          <p className="text-xs md:text-sm leading-6 text-academic-slate/70 font-sans flex items-center">
            {activeInst === "root" 
      ...
      {/* Programs Bento Grid */}
      <div className={cn(
        "mt-10 flex gap-5 overflow-x-auto snap-x snap-mandatory scrollbar-none pb-4 md:grid md:grid-cols-2 md:overflow-visible md:pb-0",
        instData.programs.length > 2 ? "lg:grid-cols-3 xl:grid-cols-4" : "lg:grid-cols-2"
          ...
          {instData.programs.map((program, index) => {
            ...
            return (
              <Reveal key={program.title} delay={index * 0.07} className="snap-center shrink-0 w-[82%] sm:w-[48%] md:w-auto md:shrink-0">
                  ...
                  <div>
                    <CardHeader className="gap-3 p-4 pb-2">
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <CardTitle className="font-serif text-2xl font-bold text-academic-slate">
                            {program.title}
                          </CardTitle>
                          <p className="mt-0.5 text-[11px] font-bold uppercase tracking-[0.15em] text-heritage-gold-strong">
                            {program.subtitle}
                          </p>
                        </div>
                        <span className={`grid h-10 w-10 shrink-0 place-items-center rounded-full bg-surface-container text-montfortian-blue transition-colors duration-300 ${hoverAccents[index % 4]} group-hover:text-white`}>
                    ...
                    <CardContent className="px-4 pb-3">
                      <p className="text-xs leading-5 text-academic-slate/75 font-sans min-h-[60px]">
                        {program.description}
                      </p>
                      ...
                      <ul className="mt-3 space-y-1.5">
                        {program.highlights.map((highlight) => (
                          <li
                            key={highlight}
                            className="flex items-center gap-2 text-[11px] text-academic-slate/70 font-sans font-medium"
                          >
                            <CheckCircle2
                    ...
                    {program.slug ? (
                      <div className="flex flex-col gap-2">
                        <Link
                          ...
                          className="inline-flex items-center gap-1 text-[11px] font-bold uppercase tracking-wider text-montfortian-blue group-hover:translate-x-1 transition-transform"
                        >
