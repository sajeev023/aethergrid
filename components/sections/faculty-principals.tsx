...
function FormerPrincipalCard({ member, index }: { member: FacultySeedMember; index: number }) {
  return (
    <Reveal key={`${member.name}-${index}`} delay={index * 0.03} className="h-full">
      <Card className="group h-full overflow-hidden bg-white transition-all duration-300 ease-out rounded-lg border-2 border-stone-texture/70 hover:border-heritage-gold/60 shadow-sm hover:shadow-panel-hover">
        <div className="relative aspect-[1054/1492] w-full overflow-hidden bg-royal-cream/30">
          {member.image ? (
            <Image
              src={member.image}
              alt={member.name}
            ...
            <div className="flex h-full w-full flex-col items-center justify-center bg-royal-cream text-academic-slate p-3 relative text-center">
              <div className="w-10 h-10 rounded-full border border-heritage-gold/30 flex items-center justify-center bg-white shadow-sm mb-2">
                <User className="h-5 w-5 text-heritage-gold-strong/60" />
              </div>
              <p className="text-[11px] font-bold text-academic-slate font-serif">{member.name}</p>
              <span className="text-[9px] font-sans font-bold uppercase tracking-widest text-heritage-gold-strong/80 mt-1">
                Image Missing
              </span>
            </div>
          )}
...
export function FacultyPrincipals({ activeInst = "lfjc" }: FacultyPrincipalsProps) {
  ...
  return (
    <section id="former-principals" className="section-texture bg-white py-8 md:py-12 overflow-hidden">
      {/* Page Hero */}
      ...
      <div className="mx-auto max-w-7xl px-5 md:px-8">
        {formerPrincipals.length > 0 ? (
          ...
          <div className="text-center py-16 text-academic-slate/50 font-sans">
            <User className="h-8 w-8 mx-auto mb-3 opacity-40" />
            <p className="text-sm">Former Principals gallery coming soon.</p>
          </div>
        )}

        {/* Silver Jubilee Archival Award Ceremony Gallery */}
        <Reveal className="mt-12 border-t border-stone-texture/40 pt-10">
          <div className="text-center max-w-2xl mx-auto mb-8">
            <span className="font-sans text-[10px] font-bold text-heritage-gold-strong uppercase tracking-[0.18em] block mb-1">
              Archival Asset (1999 Silver Jubilee)
            </span>
            <h2 className="font-serif text-2xl font-bold text-academic-slate">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {[
              ...
              <Reveal key={p.name} delay={idx * 0.05}>
                <div className="border border-stone-texture/60 bg-royal-cream/10 rounded-lg p-3.5 shadow-2xs hover:shadow-md hover:border-heritage-gold/50 transition-all duration-300 h-full flex flex-col justify-between">
                  <div>
                    <div className="relative aspect-[4/3] w-full overflow-hidden rounded mb-3 border border-stone-texture/40">
                    ...
                    <span className="text-[10px] font-sans font-bold text-heritage-gold-strong uppercase tracking-wider block mt-0.5">
                      {p.tenure}
                    </span>
                    <p className="text-[11px] font-sans text-academic-slate/75 leading-relaxed mt-2">{p.desc}</p>
                  </div>
                  <div className="mt-3 pt-2 border-t border-stone-texture/30 text-[9px] font-bold text-montfortian-blue uppercase tracking-widest font-sans">
        ...
        {/* Legacy Note */}
        <Reveal className="mt-10 border-t border-stone-texture/30 pt-8">
          <div className="max-w-2xl mx-auto text-center">
            <span className="gold-rule gold-rule-center" />
            <p className="font-editorial text-base leading-relaxed text-academic-slate/75 italic mt-4">
            ...
            <p className="mt-3 text-[11px] font-sans text-academic-slate/50 uppercase tracking-widest">
